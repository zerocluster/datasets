...
export default class CountriesTriangles extends Resources.Resource {
    ...
    async build ( tmp ) {
        ...
        const json = JSON.parse( fs.readFileSync( this.sourcePath ) ),
            { "default": area } = import( "@turf/area" ),
            { "default": earcut } = import( "earcut" ),
            dbh = await sql.new( url.pathToFileURL( tmp.path + "/" + this.files[0] ) );
