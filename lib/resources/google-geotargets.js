...
CREATE INDEX idx_geotarget_canonicalName ON "geotarget" ("canonicalName");
        ...
        await new Promise( resolve => {
            csv.parseString( data, { "headers": headers => ["id", "name", "canonical_name", "parent_id", "country", "type", "status"] } )
                ...
                .on( "data", row => {
                    ...
                    row.canonicalName = row.canonical_name;
                    delete row.canonical_name;

                    row.parentId = row.parent_id;
                    delete row.parent_id;

                    row.uule = uule.encode( row.canonicalName );
