...
class Maxmind {
    ...
    get asn () {
        if ( !READER.asn ) {
            const buf = fs.readFileSync( `${CONST.path}/${CONST.index.asn.name}.mmdb` );
    ...
    get city () {
        if ( !READER.city ) {
            const buf = fs.readFileSync( `${CONST.path}/${CONST.index.city.name}.mmdb` );
    ...
    get country () {
        if ( !READER.country ) {
            const buf = fs.readFileSync( `${CONST.path}/${CONST.index.country.name}.mmdb` );
