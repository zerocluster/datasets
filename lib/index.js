...
class Maxmind {
    ...
    get asn () {
        ...
        if ( !READERS.asn ) READERS.asn = this.#getReader( `${CONST.path}/${CONST.index.asn.name}.mmdb` );
    ...
    get city () {
        ...
        if ( !READERS.city ) READERS.city = this.#getReader( `${CONST.path}/${CONST.index.city.name}.mmdb` );
    ...
    get country () {
        ...
        if ( !READERS.country ) READERS.country = this.#getReader( `${CONST.path}/${CONST.index.country.name}.mmdb` );
    ...
    #getReader ( path ) {
        ...
        return new Reader( fs.readFileSync( path ), { cache } );
    }
        ...
        for ( const id in idx ) {
            ...
            try {
                ...
                if ( done.ok ) {
                    ...
                    idx[id].buildEpoch = this.#getBuildEpoch( id, path );
            ...
            if ( !localIndex[id] || localIndex[id].buildEpoch !== remoteIndex[id].buildEpoch ) {
                ...
                while ( 1 ) {
                    ...
                    try {
                        ...
                        localIndex[id].buildEpoch = this.#getBuildEpoch( id, path );
    ...
    #getBuildEpoch ( id, path ) {
        if ( !fs.existsSync( path ) ) return;

        const reader = this.#getReader( path );

        const buildEpoch = reader.metadata.buildEpoch.toISOString();

        // update cached reader
        if ( READERS[id] && READERS[id].metadata.buildEpoch.toISOString() !== buildEpoch ) {
            READERS[id] = reader;
        }
