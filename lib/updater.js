...
class MaxmindUpdater extends EventEmitter {
    async getIndex () {
        ...
        // download index
        try {
            const res = await fetch( `https://api.bitbucket.org/2.0/repositories/${CONST.repo}/downloads/index.json` );
    ...
    async update () {
        ...
        for ( const id in idx ) {
            ...
            if ( buildEpoch !== idx[id].buildEpoch ) {
                ...
                try {
                    const res = await fetch( `https://api.bitbucket.org/2.0/repositories/${CONST.repo}/downloads/${file}` );
    ...
    async #getBuildEpoch ( path ) {
        ...
        return reader.metadata.buildEpoch;
    }
}
