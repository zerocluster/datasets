...
class Updater extends Events {
    ...
    async #updateFromRepository ( id, remoteIndex, localIndex ) {
        ...
        // not updated
        if ( fs.existsSync( item.path ) && ( ( remoteIndex[id]?.hash && remoteIndex[id].hash === localIndex[id]?.hash ) || ( remoteIndex[id]?.lastModified && remoteIndex[id].lastModified === localIndex[id]?.lastModified ) ) ) return result( 304 );
