...
const QUERIES = {
    ...
    "get_osm_polygon": sql`SELECT id, updated, center, bbox, polygon FROM osm WHERE id = ?`.prepare(),
    "insert_osm": sql`INSERT INTO osm ( id, updated, class, type, display_name, center, bbox, polygon ) VALUES ( ?, CURRENT_TIMESTAMP, ?, ?, ?, ?, ?, ? ) ON CONFLICT ( id ) DO UPDATE SET updated = CURRENT_TIMESTAMP, class = ?, type = ?, display_name = ?, center = ?, bbox = ?, polygon = ?`.prepare(),
...
export default class extends Base {
    ...
    async API_get ( ctx, id, options ) {
        const cacheId = ( id + "" ).toUpperCase();

        var geotarget = CACHE.get( cacheId );
        ...
        if ( options.polygon ) osm = dbhCache.selectRow( QUERIES.get_osm_polygon, [geotarget.data.id] );
        ...
        if ( options.polygon ) data.polygon = osm.data.polygon;
    ...
    async #fetchOsm ( geotarget ) {
        ...
        try {
            ...
            res = dbhCache.do( QUERIES.insert_osm, [geotarget.id, osmData.class, osmData.type, osmData.display_name, osmData.center, osmData.bbox, osmData.polygon, osmData.class, osmData.type, osmData.display_name, osmData.center, osmData.bbox, osmData.polygon] );
            if ( !res.ok ) throw res;
    ...
    async #osmRequest ( url ) {
        ...
        while ( 1 ) {
            ...
            for ( const osm of json ) {
                if ( osm.class === "boundary" && osm.type === "administrative" ) {
                    res.data = osm;
