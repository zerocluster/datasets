...
const QUERIES = {
    ...
    "get_geodata": sql`SELECT id, updated, center, bbox FROM geotarget WHERE id = ?`.prepare(),
    "get_geodata_polygon": sql`SELECT id, updated, center, bbox, polygon FROM geotarget WHERE id = ?`.prepare(),
...
export default class extends Base {
    ...
    async API_get ( ctx, id, options ) {
        ...
        if ( options.polygon ) geodata = dbhCache.selectRow( QUERIES.get_geodata_polygon, [geotarget.data.id] );
        else geodata = dbhCache.selectRow( QUERIES.get_geodata, [geotarget.data.id] );

        // XXX detect not found
        if ( !geodata.data ) {
            geodata = await this.#fetchGeoData( geotarget.data );
            if ( !geodata.ok ) return geodata;
        }
