...
export default Super =>
    class extends Super {
        ...
        async API_getGeotarget ( ctx, id, options = {} ) {
            ...
            if ( !geotarget ) {
                geotarget = dbhGeoTargets.selectRow( SQL.getGeotarget, [id, id, id] );
            ...
            if ( options.geojson ) osm = dbhCache.selectRow( SQL.getOsmGeojson, [geotarget.data.id] );
            else osm = dbhCache.selectRow( SQL.getOsm, [geotarget.data.id] );
        ...
        async API_suggestGeotargets ( ctx, options ) {
            ...
            return this._read( ctx, query, { options, "dbh": dbhGeoTargets } );
        }
        ...
        async #fetchOsm ( geotarget ) {
            ...
            try {
                ...
                if ( !osmFound ) {
                    ...
                    // remove triangles
                    await dbhCache.do( SQL.deleteOsmTriangles, [geotarget.id] );
                }
                ...
                res = dbhCache.do( SQL.insertOsm, [geotarget.id, osmData.class, osmData.type, osmData.display_name, osmData.center, osmData.bbox, osmData.geojson, osmData.class, osmData.type, osmData.display_name, osmData.center, osmData.bbox, osmData.geojson] );
                if ( !res.ok ) throw res;
                ...
                if ( osmFound && ( osmData.geojson.type === "MultiPolygon" || osmData.geojson.type === "Polygon" ) ) {
                    ...
                    if ( !res.ok ) {
                        dbhCache.do( SQL.deleteOsm, [geotarget.id] );
