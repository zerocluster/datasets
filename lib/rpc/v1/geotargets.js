...
var dbhGeoTargets = await datasets.dbh( "google-geotargets" ),
...
export default class extends Base {
    constructor ( api ) {
        ...
        datasets.on( "update", async dataset => {
            if ( dataset === "google-geotargets" ) {
                dbhGeoTargets = await datasets.dbh( "google-geotargets" );
    ...
    async API_get ( ctx, id, options ) {
        ...
        if ( !geodata.data ) {
            ...
            // triangulate
            await this.app.threads.call( "worker", "triangulate", geotarget.data.id );
