...
export default class extends Base {
    ...
    async API_get ( ctx, id ) {
        ...
        res = await dbhDatasets.selectRow( QUERIES.get, [id, id, id] );
    ...
    async API_get_all ( ctx ) {
        return await dbhDatasets.select( QUERIES.get_all );
    }
    ...
    async API_get_country_random_coordinates ( ctx, id ) {
        ...
        const point = await getCountryRandomCoordinates( country.data.iso2 );
