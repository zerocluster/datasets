...
export default class extends Base {
    constructor ( api ) {
        ...
        // set update listener
        api.app.on( "update", dataset => {
            if ( dataset === "datasets" ) CACHE.reset();

            if ( dataset === "countries.geo.json" ) getCountryByCoordinates.update();
        } );
    }
    ...
    async API_get_by_coordinates ( ctx, coordinates ) {
        const id = getCountryByCoordinates.get( coordinates );
    ...
    async API_get_country_random_coordinates ( ctx, id ) {
        ...
        const point = await getCountryRandomCoordinates( this.app, country.data.iso2 );
