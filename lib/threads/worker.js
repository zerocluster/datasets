...
export default class extends Base {
    ...
    async API_triangulate ( id ) {
        var json = dbh.selectRow( sql`SELECT polygon FROM osm WHERE id = ?`, [id] ).data.polygon;

        // check features
        for ( const feature of json.features ) {
            if ( feature.geometry.type === "MultiPolygon" ) continue;
            else if ( feature.geometry.type === "Polygon" ) feature.geometry.coordinates = [feature.geometry.coordinates];
            else return result( [200, `Unable to triangulate ` + feature.geometry.type] );
        }
