...
class Maxmind {
    ...
    async update ( { force } = {} ) {
        ...
        const maxmindLicenseKey = process.env.MAXMIND_LICENSE_KEY || cfg.maxmind?.licenseKey;

        // update from maxmind
        if ( maxmindLicenseKey ) {
            let res = await this.#updateFromMaxmind( maxmindLicenseKey );
    ...
    async #updateFromMaxmind ( maxmindLicenseKey ) {
        ...
        for ( const id in idx ) {
            ...
            try {
                ...
                let res = await fetch( `https://download.maxmind.com/app/geoip_download?edition_id=${name}&license_key=${maxmindLicenseKey}&suffix=tar.gz`, { "method": "head" } );
                ...
                res = await fetch( `https://download.maxmind.com/app/geoip_download?edition_id=${name}&license_key=${maxmindLicenseKey}&suffix=tar.gz`, { "timeout": 30000 } );
