...
class Maxmind {
    ...
    async #syncRepository ( token ) {
        ...
        for ( const id in localIndex ) {
            ...
            if ( localIndex[id].lastModified !== remoteIndex[id].lastModified ) {
                ...
                const res = await this.#uploadAsset( gitHub, file + ".tar.gz", tmp );
        ...
        if ( updated ) {
            ...
            const res = await this.#uploadAsset( gitHub, "index.json", CONST.indexPath );
    ...
    async #uploadAsset ( api, filename, path ) {
        ...
        res = await api.uploadReleaseAsset( CONST.repo, releaseId, filename, fs.createReadStream( path ) );
