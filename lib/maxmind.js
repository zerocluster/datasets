...
class Maxmind {
    ...
    get asn () {
        if ( !READERS["geolite2-asn"] ) this.#initReader( "geolite2-asn", CONST.index["geolite2-asn"].path );

        return READERS["geolite2-asn"];
    }

    get city () {
        if ( !READERS["geolite2-city"] ) this.#initReader( "geolite2-city", CONST.index["geolite2-city"].path );

        return READERS["geolite2-city"];
    }

    get country () {
        if ( !READERS["geolite2-country"] ) this.#initReader( "geolite2-country", CONST.index["geolite2-country"].path );
